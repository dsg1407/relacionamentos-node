...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    if (checkInexistentProduts.length) {
      throw new AppError(
        `Could not find products ${checkInexistentProduts[0].id}`,
      );
    }

    const findProductsWithNoQuantityAvailable = products.filter(
      p =>
        existentProducts.filter(product => product.id === p.id)[0].quantity <
        p.quantity,
    );

    if (findProductsWithNoQuantityAvailable.length) {
      throw new AppError('The quantity is not available for the products');
    }
